...
# optimai宽度缩小1/2，高度保持不变（1倍）
optimai_width=$((upper_item_width/2))  # optimai宽度缩小为原来1/2
optimai_height=$upper_height  # optimai高度保持不变
...
# 4. 启动tashi（上层左侧，距离左边界30px）
osascript -e 'tell app "Terminal" to do script "cd ~/nodes && ./tashi.sh"'
sleep 1
arrange_window "tashi" $((x1+30)) $y1 $upper_item_width $upper_height

# 5. 启动optimai（上层右侧，向右偏移半个身位，宽度缩小1/2，高度不变）
osascript -e 'tell app "Terminal" to do script "cd ~/nodes && ./optimai.sh"'
sleep 1
arrange_window "optimai" $((x1+upper_item_width+spacing+upper_item_width/2)) $y1 $optimai_width $optimai_height

# 6. 启动nexus（下层左侧，高度减小30px，向下移动5px）
osascript -e 'tell app "Terminal" to do script "cd ~/nodes && ./nexus.sh"'
sleep 1
arrange_window "nexus" $x1 $nexus_ritual_y $lower_item_width $nexus_ritual_height

# 7. 启动dria（上层右侧，向右偏移半个身位，宽度缩小1/2，高度不变）
osascript -e 'tell app "Terminal" to do script "cd ~/nodes && ./dria.sh"'
sleep 1
arrange_window "dria" $((x1+lower_item_width+spacing)) $nexus_ritual_y $lower_item_width $nexus_ritual_height
